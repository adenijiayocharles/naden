...
export default function OnboardingWizard({ onComplete }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#111] border border-[#1e1e1e] rounded-xl shadow-2xl w-full max-w-md flex flex-col">
        {/* Progress bar */}
        <div className="h-1 bg-[#1e1e1e] rounded-t-xl overflow-hidden">
          <div
        ...
        <div className="px-8 py-7 flex-1">
          ...
          {step === "vault" && (
            <div className="space-y-4">
              ...
              {!skipVault && !isSetup ? (
                  ...
                  <div>
                    ...
                    {password.length > 0 && (
                      <div className="mt-1.5 flex items-center gap-2">
                        ...
                        <span className="text-xs text-[#666] w-16 text-right">{strengthLabel}</span>
                      </div>
                    )}
                  </div>
                  <input
                ...
                <div className="p-3 bg-[#0d0d0d] border border-[#1e1e1e] rounded-lg text-sm text-[#888]">
              ...
              <div className="flex gap-2 pt-2">
                <button onClick={() => setStep("welcome")} className="px-4 py-2 text-sm text-[#666] hover:text-white transition-colors">
                  Back
                </button>
                <button
          ...
          {step === "import" && (
            <div className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                <button onClick={() => setStep("vault")} className="px-4 py-2 text-sm text-[#666] hover:text-white transition-colors">
                  Back
                </button>
                <button
                  onClick={() => setShowImport(true)}
                  className="flex-1 py-2 bg-[#1a1a1a] hover:bg-[#222] border border-[#2a2a2a] text-[#ccc] text-sm rounded transition-colors"
                >
                  Import SSH config
                </button>
                <button
