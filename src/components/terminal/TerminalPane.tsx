...
const TUNNEL_STATUS_DOT: Record<TunnelStatus, string> = {
  active:     "bg-green-500",
...
function TunnelPickerRow({
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 border-b border-stroke-subtle last:border-b-0 hover:bg-surface-1 transition-colors group">
      <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${TUNNEL_STATUS_DOT[status]}`} />
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          ...
          {error && status === "error" && <span className="text-[10px] text-error truncate">{formatError(error)}</span>}
        </div>
      </div>
      <Button
        variant="ghost"
...
export default function TerminalPane({ sessionId }: Props) {
  ...
  const tunnelPickerRef = useRef<HTMLDivElement>(null);
  const allForwards = useTunnelStore((s) => s.forwards);
  const serverForwards = useMemo(
    () => allForwards.filter((f) => f.serverId === session?.serverId),
    [allForwards, session?.serverId],
  );
  ...
  const resetAddTunnelForm = useCallback(() => {
  ...
  useEffect(() => {
    if (!tunnelPickerOpen) { resetAddTunnelForm(); return; }
    const handler = (e: MouseEvent) => {
      const target = e.target as HTMLElement;
      if (!tunnelPickerRef.current?.contains(target) && !target.closest("[data-terminal-tool-trigger]")) {
        closeTool();
        resetAddTunnelForm();
      }
    };
    document.addEventListener("mousedown", handler);
    return () => document.removeEventListener("mousedown", handler);
  }, [tunnelPickerOpen, closeTool, resetAddTunnelForm]);

  const handleAddTunnel = useCallback(async () => {
  ...
  }, [addLocalPort, addFwdType, addRemoteHost, addRemotePort, session?.serverId, createTunnel, resetAddTunnelForm]);
          ...
          {showAddTunnel && (
            <div className="p-3 border-t border-stroke-subtle space-y-2.5">
                  ...
                  <>
                    ...
                    <div className="w-14 shrink-0">
                      ...
                      <Input
                        ...
                        max={65535}
                        className="h-7 text-xs px-2"
                        onKeyDown={(e) => { if (e.key === "Enter") void handleAddTunnel(); }}
                      />
                    </div>
                  </>
                )}
              ...
              <div className="flex gap-1.5">
                <Button
                  variant="secondary"
                  onClick={resetAddTunnelForm}
                  disabled={addTunnelSaving}
                  className="flex-1 h-7 text-xs"
                >
                  Cancel
                </Button>
                <Button
                  onClick={() => { void handleAddTunnel(); }}
                  disabled={addTunnelSaving}
                  className="flex-1 h-7 text-xs"
                >
