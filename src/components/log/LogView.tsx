...
import { logCommands, sessionLogCommands } from "../../lib/tauriCommands";
...
function RecordingsPanel() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const rows = await sessionLogCommands.listSessionLogs(filterServer || undefined);
      setLogs(rows);
    } catch (e) {
  ...
  useEffect(() => { void load(); }, [filterServer]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {error && <p className="text-sm text-error px-5 py-2 border-b border-stroke-subtle shrink-0">{error}</p>}
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-1 justify-end">
                      <Button
                        variant="ghost"
                        size="icon-xs"
                        onClick={() => void sessionLogCommands.revealSessionLog(log.filePath)}
