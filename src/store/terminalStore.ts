...
export const useTerminalStore = create<TerminalStore>((set, get) => {
  // Marks the session "disconnected" with a countdown, then re-opens it under a
  // new session ID once `RECONNECT_DELAYS[attempt]` elapses — used both for the
  // first drop after a successful connection and for each subsequent retry.
  function scheduleReconnect(sessionId: string, attempt: number) {
    ...
    const timer = setTimeout(async () => {
      ...
      if (newId) autoReconnectSessions.set(newId, attempt);
    }, delay);
      ...
      const [, ...unlisteners] = await Promise.all([
        ...
        listen<boolean>(`terminal:closed:${sessionId}`, ({ payload: isClean }) => {
          ...
          if (autoReconnectSessions.has(sessionId)) {
            ...
            scheduleReconnect(sessionId, nextAttempt);
            return;
          }
